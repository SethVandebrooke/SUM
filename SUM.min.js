function $SUM(name, uid) {
    var app = this; app.uid = uid || "username"; if (localStorage.getItem("uid") != null) { app.uid = localStorage.getItem("uid") }
    ...
    app.users = (function () {
        ...
        return new dataGroup(name + "-app-data", { set: function (a, b) { return localStorage.setItem(a, b) }, get: function (a) { return localStorage.getItem(a) } })
    })(); app.register = function (data, done, fail) {
        if (data[app.uid]) {
            if (data.password) {
                if (!app.users.exists(app.uid, data[app.uid])) {
                data.password = hash(data.password)
                    app.users.add(data); done(data)
                } else { fail(data, "That " + app.uid + " is already in use!") }
            } else { fail(data, "Invalid submission. Password required."); throw new Error(name + " -> Register: 'password' is a required property") }
        } else { fail(data, "Invalid submission. " + app.uid + " required."); throw new Error(name + " -> Register: " + app.uid + " must be defined.") }
    }; app.update = function (data, id, finished) {
        for (var k in data) { if (k == "password") { app.users.edit(app.uid, id, k, hash(data[k])) } else { app.users.edit(app.uid, id, k, data[k]) } }
        react(finished, data)
    }; app.auth = function (data, done, fail) { if (data[app.uid]) { if (data.password) { if (app.users.exists(app.uid, data[app.uid])) { if (app.users.get(app.uid, data[app.uid]).password == hash(data.password)) { react(done, data) } else { fail(data, "Incorrect password!") } } else { fail(data, "User does not exist!") } } else { fail(data, "Invalid authentication attempt. Password required."); throw new Error(name + " -> Auth: 'password' is a required property") } } else { fail(data, "Invalid authentication attempt. " + app.uid + " required."); throw new Error(name + " -> Auth: " + app.uid + " must be defined.") } }; app.login = function (data, done, fail) { app.auth(data, function (data) { sessionStorage.setItem(name + "-current-user", data[app.uid]); react(done, data) }, function (data) { react(fail, data) }) }; app.logout = function (done) { sessionStorage.removeItem(name + "-current-user"); window.location.href = done }; app.loggedInAs = function () { return sessionStorage.getItem(name + "-current-user") || "User" }; app.loggedIn = function () { return sessionStorage.getItem(name + "-current-user") != null ? !0 : !1 }; app.currentUser = function () { return app.users.get(app.uid, app.loggedInAs()) }; app.getForm = function (form) {
        var response = {}; var elements = form.elements; for (var i = 0; i < elements.length; i++) {
            if (elements[i].tagName == "INPUT" || elements[i].tagName == "SELECT" || elements[i].tagName == "TEXTAREA") {
                var backup = elements[i].tagName + Math.floor(Math.random() * 500); var title = elements[i].name != "" ? elements[i].name : (elements[i].id != "" ? elements[i].id : backup); var val = elements[i].value; if (!elements[i].tagName == "INPUT" && !elements[i].type == "submit") { elements[i].value = "" }
                response[title] = val
            }
        }
        return response
    ...
    var app = new $SUM("defaultApp"); if (document.querySelectorAll("html")[0].getAttribute("force-login") ? true : !1) { if (!app.loggedIn()) { window.location.href = document.querySelectorAll("html")[0].getAttribute("force-login") } }
    var forms = document.querySelectorAll("form"); forms.forEach(e => {
        e.addEventListener("submit", function (e2) {
            if (e.getAttribute("type") === "login") {
                e2.preventDefault(); var id = e.getAttribute("uid"); if (id != null) { app.uid = id; localStorage.setItem("uid", id) }
            ...
            if (e.getAttribute("type") === "update") { e2.preventDefault(); var formData = app.getForm(e); app.update(formData, app.loggedInAs(), e.getAttribute("on-success") || window.location.href) }
        }); if (e.getAttribute("type") === "update") { var user = app.currentUser(); for (var k in user) { e.querySelectorAll("#" + k + ", [name='" + k + "']").forEach(a => { if (k != "password") { a.setAttribute("value", user[k]) } }) } }
    }); var logout = document.querySelectorAll("[logout]"); logout.forEach(e => { e.addEventListener("click", function (e2) { app.logout(e.getAttribute("logout")) }) }); var user = app.currentUser(); for (var k in user) { var elem = document.querySelectorAll(k); console.log("element: ", elem); elem.forEach(e => { e.innerHTML = user[k] }) }
})(window)